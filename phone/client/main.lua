...
local function AnswerCall()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = 'Phone',
                text = "Você não tem nenhuma ligação pendente...",
                icon = 'fas fa-phone',
                color = '#e84118',
            },
        })
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.TriggerCallback('qb-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
                ...
                SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { timeout = 3000, title = 'Bank', text = 'Conta não existe!', icon = 'fas fa-university', color = '#ff0000', }, })
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { title = 'QBank', text = '&#36;' .. amount .. ' foi adicionado à sua conta!', icon = 'fas fa-university', color = '#8c7ae6', }, })
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = 'Advertisement',
            text = 'Um novo anúncio foi postado por ' .. LastAd,
            icon = 'fas fa-ad',
            color = '#ff8f1a',
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:CancelCall', function()
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({
            ...
            NotifyData = {
                title = 'Phone',
                content = 'A ligação foi encerrada',
                icon = 'fas fa-phone',
                timeout = 3500,
                color = '#e84118',
            },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = 'Bank',
                text = '$' .. amount .. ' foi retirado do seu saldo!',
                icon = 'fas fa-university',
                color = '#ff002f',
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = 'Phone',
            text = 'Você tem uma nova sugestão!',
            icon = 'fa fa-phone-alt',
            color = '#04b543',
            timeout = 1500,
        },
...
RegisterNetEvent('qb-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = 'Phone',
                text = "Você não tem uma ligação recebida...",
                icon = 'fas fa-phone',
                color = '#e84118',
            },
        })
